...
STAGE1_MAPPING: Dict[str, Union[str, Callable[[Dict[str, Any]], Any]]] = {
    ...
    "esxi_hosts_info": lambda g: (
        [
            {
                "user": "root",
                "password": g.get("esxi_password", "Wetcom01!"),
                "address": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201))}"
            },
            {
                "user": "root",
                "password": g.get("esxi_password", "Wetcom01!"),
                "address": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201)) + 1}"
            }
        ]
        if not g.get("esxi_hosts", {}).get("info")
        else [
            {
                "user": g.get("esxi_hosts", {}).get("user", "root"),
                "password": g.get("esxi_hosts", {}).get("password", g.get("esxi_password", "Wetcom01!")),
...
STAGE3_MAPPING: Dict[str, Union[str, Callable[[Dict[str, Any]], Any]]] = {
    ...
    "iscsi_network_config": lambda g: (
        [
            {
                "host_address": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201))}",
                "iscsi_ip": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201)) + 2}",
                "iscsi_netmask": g.get("esxi_netmask", "255.255.255.0")
            },
            {
                "host_address": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201)) + 1}",
                "iscsi_ip": f"{g.get('esxi_ip_subnet', '10.106.3')}.{int(g.get('esxi_ip_start_offset', 201)) + 3}",
                "iscsi_netmask": g.get("esxi_netmask", "255.255.255.0")
            }
        ]
        if not g.get("esxi_hosts", {}).get("info")
        else [
            {
                ...
                "iscsi_netmask": g.get("common_network", {}).get("network_netmask", g.get("esxi_netmask", "255.255.255.0"))
            }
            for host in g.get("esxi_hosts", {}).get("info", [])
        ]
    )
...
def get_nested_val(data: Dict[str, Any], path: str) -> Any:
    """Resolves a dot-separated path in a nested dictionary, with flat fallback."""
    parts = path.split('.')
    current = data
    found_nested = True
    for part in parts:
    ...
    if found_nested and current is not None:
        return current
    ...
    flat_key = key_map.get(path)
    if flat_key and isinstance(data, dict) and flat_key in data and data[flat_key] is not None:
        return data[flat_key]
        
    last_part = parts[-1]
    if isinstance(data, dict) and last_part in data and data[last_part] is not None:
